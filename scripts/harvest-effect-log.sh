...
# ---- サニタイズ検証（コミット前ゲート） --------------------------------------
# 集計 md に固有名・SHA が混入していないことを機械 grep で確認する。
# 指摘本文はスキーマ上そもそも生ログに存在しない（findings は件数のみ）ため構造的に 0 件。
verify_sanitized() {
  f="$1"
  bad=0
  # 40 桁以上の hex = 生 SHA の混入疑い（repo_bucket は 12 桁なので許容される）
  if grep -nEq '[0-9a-f]{40,}' "$f" 2>/dev/null; then
    echo "SANITIZE NG: 40 桁以上の hex（SHA 疑い）を検出:" >&2
    grep -nE '[0-9a-f]{40,}' "$f" >&2
    bad=1
  fi
  # 対象リポの basename 実名の混入（集計 md は repo_bucket ハッシュのみで区別すべき）
  for name in iskwyuki-claude-plugins tech-blog Antenna portfolio keiba; do
    if grep -nFq "$name" "$f" 2>/dev/null; then
...
# 生ログを収集（該当月のみ・妥当な JSON 行のみ）。読み取り専用。
FOUND_REPOS=0
for r in $REPOS; do
...
SUMMARY=$(jq -s '{
  total: length,
  repos: ([.[].repo_bucket] | unique | length),
...
# ゲート理由別・モデル別・diff 分布の表（tab 区切り → md 行）
gate_reason_rows() {
  jq -rs 'map(select(.tool=="pre-commit-gate")) | group_by(.gate.reason_category)
    | map("| \(.[0].gate.reason_category) | \(map(select(.gate.blocked==true))|length) | \(map(select(.gate.blocked==false))|length) |")
    | .[]' "$TMP"
}
model_rows() {
  jq -rs 'map(select(.model!="")) | group_by(.model)
    | map("| \(.[0].model) | \(length) | \(map(.findings.critical)|add) | \(map(.findings.warning)|add) | \(map(.findings.info)|add) | \(map(.verified_confirmed)|add) | \(map(.refuted)|add) |")
    | .[]' "$TMP"
}
diff_rows() {
  jq -rs 'group_by(.diff_size_bucket)
    | map("| \(.[0].diff_size_bucket) | \(length) |")
    | .[]' "$TMP"
}
...
# md 組み立て（集計値のみ。repo 実名・SHA・指摘本文は一切含めない）
{
  ...
  echo "- **サニタイズ検証**: 固有名 0 件・SHA 0 件・指摘本文 0 件（機械 grep 済み・コミット前ゲート通過）"
