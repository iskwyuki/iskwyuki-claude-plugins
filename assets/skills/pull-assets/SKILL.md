...
- 同期後は `git status -- .claude/` を見せ、**同期で書き込んだパスだけ**を明示 add（`git add .claude/<type>/...` を走査結果から列挙）して `git commit -m "chore: iskwyuki-claude-plugins 同期"` を案内する。`git add .claude/` 一括は使わない（同期対象外の `.claude/memory/` やプロジェクト固有 skill の WIP を巻き込むため）
