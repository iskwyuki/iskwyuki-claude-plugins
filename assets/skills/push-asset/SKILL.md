...
3. `--auto-merge` 時: pr skill の**通行証プロトコル**に従い PR を作成（`touch .claude/pr-skill-active` を独立した Bash 呼び出しで → `gh pr create ... ; rm -f .claude/pr-skill-active` を同一呼び出しで）→ `gh pr merge --squash --delete-branch` → local main を `--ff-only` で同期。指定なしなら PR 作成コマンドを案内し、ユーザーが GitHub 上で確認してから merge する
