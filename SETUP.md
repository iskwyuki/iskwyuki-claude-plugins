...
## 外部由来の選別ミラー skill（grill-me / zoom-out / prototype）

[mattpocock/skills](https://github.com/mattpocock/skills)（MIT）から、harness・自前 skills と役割も生成物も衝突しないものだけを選別して `assets/skills/` にミラーしています。プラグイン丸ごと同梱（15 skills）は tdd / triage / to-issues 等が harness と重複するため不採用。grill-with-docs も CONTEXT.md / ADR を生成して harness の SSOT 管理と競合するため見送りました。
