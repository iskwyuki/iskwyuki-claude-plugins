...
## 外部由来の選別ミラー skill（grill-me / grill-with-docs）

[mattpocock/skills](https://github.com/mattpocock/skills)（MIT）から、harness・自前 skills と役割が衝突しないものだけを選別して `assets/skills/` にミラーしています。プラグイン丸ごと同梱（15 skills）は tdd / triage / to-issues 等が harness と重複するため不採用としました。

- `/grill-me` — 計画・設計を 1 問ずつ（推奨回答付きで）尋問し、決定木の全分岐を解消する
- `/grill-with-docs` — 計画を既存ドキュメント（用語集・ADR 等）と突き合わせて尋問し、ドキュメントもインライン更新する

上流追従は GitHub Action（`.github/workflows/sync-upstream-skills.yml`、毎日実行）が担い、上流に差分が出ると自動で同期 PR が作成されます。対象ファイルは `.github/upstream-skills.manifest` で管理します。ミラーは verbatim（無改変）で、出典・ライセンスは各 skill ディレクトリの `NOTICE.md` に記載しています。
...
| `/grill-with-docs` | 既存ドキュメントと突き合わせる尋問＋ドキュメント更新（mattpocock/skills 由来） |
