...
従来の `/plugin marketplace update` → `/pull-assets` の 2 step を本 skill 内で完結させる。`iskwyuki-claude-plugins` 本体の更新を適用した場合のみ実行する（harness 等の外部 plugin のみの更新では不要）。

1. **配信元リポジトリ自身ではスキップ**する（`.claude/skills` が `assets/skills` への symlink で運用されており、コピー同期は不要かつ有害）:
   ```bash
   [ -L .claude/skills ] && echo "SKIP: 配信元リポジトリ（symlink 運用）"
   ```
2. 更新後のキャッシュパスを**再解決**する（`claude plugin update` でバージョンディレクトリが変わるため。asset の実体は再起動前でも新版がディスクに展開済み）:
   ```bash
   ...
   ```
3. 以降は pull-assets skill と同一フロー（詳細はそちらを正とする）: `ls -1 "$PLUGIN_ROOT/assets/"` で対象を動的走査 → `rsync -avn`（dry-run）で新規/上書きの差分を提示 → ユーザー確認後に `rsync -av` で実コピー（**`--delete` は使わない**）
4. 同期後は `git status -- .claude/` を提示し、`git add .claude/ && git commit -m "chore: iskwyuki-claude-plugins 同期"` を案内する
