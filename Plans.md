...
| 6.5 | **[Stage 3: 実証・横展開]** 実 app プロジェクト 1 つ（Antenna 等・起動対象あり）に `.wt-parallel.yaml` を置き起動込みで実走。docs 整備（`docs/wt-parallel.md`）・利用者向け README への導線追加・pull-assets 配布確認 [tdd:skip:docs-only] | 実 app プロジェクトで worktree 作成→起動→health 緑→停止→破棄が一巡した記録がある。docs マージ・CLAUDE.md/README から参照 | 6.3 | cc:完了（実 app Antenna〔Next.js+Prisma〕で一巡を実走: wt-new→worktree〔.env 引き継ぎ〕→wt-up〔pre_start pnpm install→next dev を offset ポートで起動→health 緑〔port-open フォールバック: Prisma 未生成で / は 500 でもサーバ起動を検知〕→URL/ログ提示〕→wt-down〔停止・ポート解放〕→wt-rm〔破棄〕を **Antenna に痕跡ゼロ**で完了。docs/wt-parallel.md 新規＋README/CLAUDE.md 導線追加。pull-assets 配布は動的走査で自動対象・rsync 実配布シミュレーション検証済み。docs-only につき plugin.json bump なし。実証で post_create 未実装を発見→6.6 起票）|
| 6.6 | **[Stage 2 補完]** `wt-new` に `post_create` フック実行を追加（作成＋設定/plugin 引き継ぎ直後に実行・失敗で中断・§6）。実 app 横展開で worktree の依存初期化（`pnpm install` 等）に必要。現状 4 フックのうち post_create のみ未配線 [tdd:required] | `post_create` 宣言時に worktree 作成直後へ注入済み env で実行される回帰テスト。失敗時は中断。plugin.json bump | 6.1 | cc:完了（wt-new の plugin 登録直後に post_create 実行を追加。WT_SLUG/WT_OFFSET〔作成時未採番のため 0 既定〕+env マップを注入・worktree ルートで sh -c 実行・失敗は wt_die で中断〔半端 worktree は wt-rm 案内〕・manifest strict-subset 外も中断。TDD Red→Green〔一時退避で Red 実証。テスト初回で WT_SLUG 未 export バグ〔変数名 SLUG〕を検出・修正〕。test-wt-lifecycle に post_create 実行＋WT_SLUG 注入・失敗中断を追加。これで 4 フック〔post_create/pre_start/post_start/pre_rm〕が全配線。0.18.0）|
