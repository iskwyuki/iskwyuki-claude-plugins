...
| 1.4 | harvest-lessons の会社リポジトリ実走。データ源の主軸（2026-06-13 確定）。SSO 認可・対象 2 リポジトリの read アクセス確認は完了済み（名称・アクセス手順はローカル manifest ~/dev/quality-baseline-private/ 参照）。読み取り専用で収穫 → サニタイズ検証 [tdd:skip:docs-only] | 教訓が汎用化されて保存され、報告に「サニタイズ検証済み: 固有名 0 件・コード一致 0 件」が明記されている | 1.3 | cc:TODO |
