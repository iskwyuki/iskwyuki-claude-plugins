...
| 6.6 | **[Stage 2 補完]** `wt-new` に `post_create` フック実行を追加（作成＋設定/plugin 引き継ぎ直後に実行・失敗で中断・§6）。実 app 横展開で worktree の依存初期化（`pnpm install` 等）に必要。現状 4 フックのうち post_create のみ未配線 [tdd:required] | `post_create` 宣言時に worktree 作成直後へ注入済み env で実行される回帰テスト。失敗時は中断。plugin.json bump | 6.1 | cc:TODO |
