...
> また、[mattpocock/skills](https://github.com/mattpocock/skills)（MIT）から harness と役割が衝突しない `/grill-me` / `/grill-with-docs` だけを選別して `assets/skills/` にミラーしています。上流追従は GitHub Action が毎日チェックし、差分があれば同期 PR を自動作成します。
