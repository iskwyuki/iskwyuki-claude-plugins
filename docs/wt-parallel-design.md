...
業務で使う `wt-parallel-dev-kit`（1,188 行、Postgres 実データ複製・process-compose 起動が app スタック固有）を**設計参考**とする。kit のうち **worktree ライフサイクル（作成・設定/plugin 引き継ぎ・破棄・ポート採番）は汎用化可能**で、app 固有部分（DB 複製・依存同期・起動コマンド）はマニフェスト＋フックに externalize する。
...
- 孤児 DB 掃除（kit の `dev-prune-db.sh` 相当）は DB 密結合ゆえ汎用版に持たない。
...
1. **マニフェスト詳細スキーマ** — フィールド名・必須/任意・YAML パーサ（bash からどう読むか）。Task 6.2 で確定。
2. **`.dev/` の gitignore 追記** — 導入時に `wt-new`/skill が `.gitignore` へ自動追記するか、bootstrap で設定するか。
3. **health url のポート展開の実装詳細** — `env` マップ展開後に `${...}` を安全に解決する手順（シェル式評価のサンドボックス）。
4. **YAML 依存** — `yq` 前提にするか、最小サブセットを自前パースするか（前提ツールを増やさない方針との兼ね合い）。
